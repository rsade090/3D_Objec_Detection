...
dataset = KittiDataset(configs, mode='train', lidar_aug=None, hflip_prob=0., num_samples=configs.num_samples)

print('\n\nPress n to see the next sample >>> Press Esc to quit...')
for id, data in enumerate(dataset):
    metadata, fov, bev, img, targets = data
    top_view, top_img = bev
    front_depth, front_intensity, front_height = fov
    # show the images 
    plt.subplot(1, 4, 1)
    plt.imshow(img)
    plt.subplot(1, 4, 2)
    plt.imshow(top_img)
    plt.subplot(1, 4, 3)
    plt.imshow(front_depth)
    plt.subplot(1, 4, 4)
    plt.imshow(front_intensity)
    plt.show()
